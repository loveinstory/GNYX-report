...
27. P13 套餐名称确定为“年轻力精准评估健康管理报告”，模板来源路径为 `D:\AWK-OCR\figma-plugin-figma-openai-curated-1\outputs\html-report\P13`；当前已完成模板工程化、字段配置、规则、OCR 策略和 AI 提示词，并基于 `260601_WTFY_1012_20260601185401505.pdf` 及其 JSON OCR 输出将策略升级为 `P13-ocr-strategy-v0.2-telomere-json-pdf`，支持端粒报告 `report_info + patient_info + test_results` JSON 输入和 PDF 文本层回退解析；第 07 页调用 `assets/images/p07-food-pyramid.png` 食物金字塔图片。
28. P14 套餐名称确定为“安康御癌专项评估健康管理报告”，模板来源路径为 `D:\AWK-OCR\figma-plugin-figma-openai-curated-1\outputs\html-report\P14`；当前已完成模板工程化、关键字段配置、规则草案、OCR 策略占位和 AI 提示词接入，并完成第01-04、06-08、11页关键 `data-field` 绑定；待拿到真实或脱敏样例后继续升级 CDA、PTF、CTF、五癌甲基化、CTC 和连续复测趋势图的专项 OCR 与 AI 联调。
...
| P13 | 年轻力精准评估健康管理报告 | 研发中 | 已接入 P13 HTML 模板、字段配置、规则、OCR 策略和 AI 提示词；OCR 升级至 `P13-ocr-strategy-v0.2-telomere-json-pdf`，支持端粒报告 JSON OCR 与同版 PDF 文本层解析，核心覆盖端粒年龄、实际年龄、端粒相对长度、Ct 值和同龄人群百分位；第 07 页调用 `p07-food-pyramid.png` 食物金字塔图片 |
| P14 | 安康御癌专项评估健康管理报告 | 研发中 | 已接入 P14 HTML 模板、关键字段配置、规则草案、OCR 策略占位和 AI 提示词；已完成第01-04、06-08、11页关键字段绑定，待真实样例完善 CDA/PTF/CTF/甲基化/CTC OCR 与趋势图联动 |
| P15 | 待确认 | 未启动 | 需要提供套餐名称和检验项目 |
| P16 | 药物基因组学评估健康管理报告 | 研发中 | 已接入 P16 HTML 模板、字段配置、规则、OCR 策略和 AI 提示词；OCR 升级至 `P16-ocr-strategy-v0.2-pgx-multireport-json-pdf`，支持 `reports[]` 多报告聚合 JSON OCR 结构，并在解析 `hebing44831.pdf` 时优先读取同目录 `OCR.txt` 作为结构化 sidecar 输入 |
